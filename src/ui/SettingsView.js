...
export class SettingsView extends Component {
  ...
  initEvents() {
    this.registerUnsub(events.on('settings:updated', () => this.render()));
    this.registerUnsub(events.on('db:backupRestored', () => this.render()));
  ...
  render() {
    ...
    const statsSummary = practiceTrackerService.getStatsSummary();
    ...
    this.container.innerHTML = `
      <div class="settings-view-wrapper" role="region" aria-label="Ajustes y Perfil">
        ...
        <div class="settings-sections-list">
          ...
          <!-- 3. Preferencias del Músico e Instrumento -->
          <div class="settings-card-group">
  ...
  bindEvents() {
    ...
    // Exportar Backup con BackupSyncEngine
    this.container.querySelector('#btnExportBackup')?.addEventListener('click', async () => {
      await backupSyncEngine.exportFullBackup();
    });

    // Importar Backup con BackupSyncEngine
    ...
    fileInput?.addEventListener('change', async (e) => {
      ...
      if (!file) return;
      await backupSyncEngine.importFullBackup(file);
      this.render();
    });
  }
}
